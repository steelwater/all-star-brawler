...
const CROUCHING_HEIGHT := 58.0
const STANDING_WEAPON_MOUNT := Vector2(20, -24)
const CROUCHING_WEAPON_MOUNT := Vector2(20, 22)
const STANDING_KICK_WEAPON_MOUNT := Vector2(8, -18)
const CROUCHING_KICK_WEAPON_MOUNT := Vector2(18, 20)
const DEFENSE_ARC_RADIUS := 65.0
const DAMAGE_FLASH_DURATION := 0.5
const DAMAGE_BLINK_INTERVAL := 0.08
const DAMAGE_FLASH_COLOR := Color("b00020")
...
func _apply_command(command: FighterCommand, delta: float) -> void:
	...
	is_crouching = command.crouch and can_ground_action
	is_defending = command.defend and can_ground_action and not is_crouching
	if not command.taunt:
		is_taunting = false
	_update_collision_shape()
	var direction := clampf(command.move_axis, -1.0, 1.0)
	if is_defending or is_crouching or taunt_remaining > 0.0:
		direction = 0.0
	...
	if command.jump and can_ground_action and not is_defending and not is_crouching:
		velocity.y = JUMP_VELOCITY
	if command.attack and not is_defending and not is_crouching and taunt_remaining <= 0.0 and weapon != null:
		weapon.try_attack(self, facing)
	if command.kick and not is_defending and taunt_remaining <= 0.0:
		_try_kick()
	if command.weapon_action and not is_defending and taunt_remaining <= 0.0:
		_throw_or_pickup_weapon()
	if command.taunt and can_ground_action and not is_defending and not is_crouching:
		is_taunting = true
		_start_taunt()
	_update_weapon_mount_pose()
	weapon_mount.visible = not is_taunting
...
func _update_collision_shape() -> void:
	...
	if capsule == null:
		return
	var wanted_height := CROUCHING_HEIGHT if is_crouching else STANDING_HEIGHT
	if not is_equal_approx(capsule.height, wanted_height):
		capsule.height = wanted_height
		collision_shape.position.y = 17.0 if is_crouching else -2.0


func _update_weapon_mount_pose() -> void:
	if kick_flash_remaining > 0.0:
		var kick_mount := CROUCHING_KICK_WEAPON_MOUNT if is_crouching else STANDING_KICK_WEAPON_MOUNT
		weapon_mount.position = Vector2(facing * kick_mount.x, kick_mount.y)
		weapon_mount.rotation = -facing * PI * 0.5
	else:
		weapon_mount.position = CROUCHING_WEAPON_MOUNT if is_crouching else STANDING_WEAPON_MOUNT
...
func is_damage_blink_visible() -> bool:
	if damage_flash_remaining <= 0.0:
		return false
	return fmod(damage_flash_elapsed, DAMAGE_BLINK_INTERVAL * 2.0) < DAMAGE_BLINK_INTERVAL


func _draw() -> void:
	var body_color := fighter_color.darkened(0.4) if is_defending else fighter_color
	if is_damage_blink_visible():
		body_color = DAMAGE_FLASH_COLOR
	...
	if kick_flash_remaining > 0.0:
		if is_crouching:
			_draw_crouching_kick_pose(body_color)
		else:
			_draw_standing_kick_pose(body_color)
	elif is_crouching:
		draw_circle(Vector2(0, 8), 18.0, body_color.lightened(0.18))
		draw_rect(Rect2(-28, 20, 56, 26), body_color)
		draw_line(Vector2(-16, 24), Vector2(-35, 41), body_color, 10.0)
		if weapon == null:
			draw_line(Vector2(16, 24), Vector2(35, 41), body_color, 10.0)
	else:
	...
	if is_defending:
		...
		draw_arc(Vector2(facing * 20, -18), DEFENSE_ARC_RADIUS, shield_angle - 0.75, shield_angle + 0.75, 18, Color(1, 1, 1, 0.65), 4.0)


func _draw_standing_kick_pose(body_color: Color) -> void:
	var pose := PackedVector2Array([
		Vector2(facing * -8, -36),
		Vector2(facing * 15, -23),
		Vector2(facing * 27, 10),
		Vector2(0, 27),
		Vector2(facing * -22, -4),
	])
	draw_colored_polygon(pose, body_color)
	draw_circle(Vector2(facing * -18, -52), 18.0, body_color.lightened(0.18))
	draw_line(Vector2(facing * -12, -22), Vector2(facing * -32, 5), body_color, 10.0)
	draw_line(Vector2(0, 20), Vector2(0, 48), body_color.darkened(0.15), 14.0)
	draw_line(Vector2(facing * 16, 10), Vector2(facing * 58, 4), body_color.lightened(0.12), 14.0)


func _draw_crouching_kick_pose(body_color: Color) -> void:
	draw_circle(Vector2(facing * -8, 7), 18.0, body_color.lightened(0.18))
	draw_rect(Rect2(-25, 20, 50, 26), body_color)
	draw_line(Vector2(facing * -15, 24), Vector2(facing * -36, 43), body_color, 10.0)
	draw_line(Vector2(facing * 12, 34), Vector2(facing * 58, 34), body_color.lightened(0.12), 14.0)
